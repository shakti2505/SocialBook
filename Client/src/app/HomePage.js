...
const HomePage = (props) => {
  ...
  const submitPostImages = async (e) => {
    try {
      ...
      formData.append('upload_preset', process.env.REACT_APP_Cloudnary_upload_preset);
      formData.append('folder', process.env.REACT_APP_folder);
      formData.append('cloud_name', process.env.REACT_APP_cloud_name);
      const uploadPromises = postImages.map(async (item) => {
        formData.append('file', item);
        let response1 = await fetch(`https://api.cloudinary.com/v1_1/${process.env.REACT_APP_Cloudnary_upload_preset}/image/upload`, {
          method: "post",
          body: formData
        });
        if (!response1.ok) {
      ...
      <div className='d-flex'>
        ...
        <div className='col-5 offset-1' id='middleBar' style={{ overflowY: 'hidden' }}>
          ...
          <div>
            ...
            <div className=' d-flex justify-content-center mt-3'>
              <div className='Post-form p-2'>
                <div className='d-flex  mx-2'>
                  ...
                  <button onClick={OpenPostModal} className='searchBtn btn btn-light mx-2 w-100 d-flex justify-content-start'>What's on you mind, User?</button>
                </div>
                <hr />
            ...
            <PostModal convertedProfilePicture={UserProfilePicture} uploadedPost={posts ? posts : []} />
            {
              <Modal
                {...props}
                size="md"
              ...
              >
                ...
                <Modal.Body style={{ maxHeight: '25rem', overflowY: "auto", position: 'relative' }}>
                  ...
                  <div
                    ...
                    <button onClick={() => setShowAddPhotoModal(false)} type="button" class="btn-close mx-2 mt-2" aria-label="Close" style={{ float: "right" }}></button>
                    <div
                  ...
                  {imagePreviews.length !== 0 &&
                    <div className={imagePreviews.length > 1 ? 'd-flex flex-row vh-50' : 'w-100'} style={{ borderRadius: "10px", border: "1px solid lightgrey", position: 'relative' }}>

                      {
                        imagePreviews.length !== 0 && imagePreviews.slice(0, 3).map((previewUrl, index) => (
                          <>
                            <div style={{ position: 'relative', width: "100%" }} >
                              {
                                index === 0 && <button style={{ position: 'absolute', top: '10px', left: '10px', display: 'flex', zIndex: '1' }} className='position-absolute btn btn-light'>
                                  <input
                                    id='FileUpload'
                                    style={{ zIndex: "1", opacity: "0", border: "1px solid", top: '0.5px', left: '0.5px' }}
                              ...
                              <img key={index + Math.random} className='p-1' src={imagePreviews.length === 1 ? imagePreviews[0] : previewUrl} style={{ height: "100%", width: "100%", borderRadius: '10px', objectFit: 'cover' }} />
                              {imagePreviews.length >= 3 && index === 2 && (
                                <h1 style={{ position: 'absolute', bottom: '25%', left: '25%', color: 'white', color: 'white' }}>+{imagePreviews.length - 3}</h1>
                              )}
                            </div>
                          </>
                        ))}
                  ...
                  {/* PhotoPreview Ends */}
                  <div id='addToYourPost' className='d-flex justify-content-around mt-1'>
                    <button className='btn btn-link addPost m-1'>Add To Your Post</button>
                    <div className='PostBtns m-1'>
                      <button onClick={() => setShowAddPhotoModal(true)} className='btn btn-link'> <svg xmlns="http://www.w3.org/2000/svg" height="25" width="25" viewBox="0 0 576 512"><path fill="#46BD63" d="M160 32c-35.3 0-64 28.7-64 64V320c0 35.3 28.7 64 64 64H512c35.3 0 64-28.7 64-64V96c0-35.3-28.7-64-64-64H160zM396 138.7l96 144c4.9 7.4 5.4 16.8 1.2 24.6S480.9 320 472 320H328 280 200c-9.2 0-17.6-5.3-21.6-13.6s-2.9-18.2 2.9-25.4l64-80c4.6-5.7 11.4-9 18.7-9s14.2 3.3 18.7 9l17.3 21.6 56-84C360.5 132 368 128 376 128s15.5 4 20 10.7zM192 128a32 32 0 1 1 64 0 32 32 0 1 1 -64 0zM48 120c0-13.3-10.7-24-24-24S0 106.7 0 120V344c0 75.1 60.9 136 136 136H456c13.3 0 24-10.7 24-24s-10.7-24-24-24H136c-48.6 0-88-39.4-88-88V120z" /></svg></button>
                    </div>
                    <div className='PostBtns m-1'>
                      <button className='btn btn-link'><svg xmlns="http://www.w3.org/2000/svg" height="25" width="25" viewBox="0 0 640 512"><path fill="#0D6EFD" d="M224 256A128 128 0 1 0 224 0a128 128 0 1 0 0 256zm-45.7 48C79.8 304 0 383.8 0 482.3C0 498.7 13.3 512 29.7 512H418.3c10 0 18.8-4.9 24.2-12.5l-99.2-99.2c-14.9-14.9-23.3-35.1-23.3-56.1v-33c-15.9-4.7-32.8-7.2-50.3-7.2H178.3zM384 224c-17.7 0-32 14.3-32 32v82.7c0 17 6.7 33.3 18.7 45.3L478.1 491.3c18.7 18.7 49.1 18.7 67.9 0l73.4-73.4c18.7-18.7 18.7-49.1 0-67.9L512 242.7c-12-12-28.3-18.7-45.3-18.7H384zm24 80a24 24 0 1 1 48 0 24 24 0 1 1 -48 0z" /></svg></button>
                    </div>
                    <div className='PostBtns m-1'>
                      <button className='btn btn-link'>  <svg xmlns="http://www.w3.org/2000/svg" height="25" width="25" viewBox="0 0 512 512"><path fill="#FFD43B" d="M464 256A208 208 0 1 0 48 256a208 208 0 1 0 416 0zM0 256a256 256 0 1 1 512 0A256 256 0 1 1 0 256zm177.6 62.1C192.8 334.5 218.8 352 256 352s63.2-17.5 78.4-33.9c9-9.7 24.2-10.4 33.9-1.4s10.4 24.2 1.4 33.9c-22 23.8-60 49.4-113.6 49.4s-91.7-25.5-113.6-49.4c-9-9.7-8.4-24.9 1.4-33.9s24.9-8.4 33.9 1.4zM144.4 208a32 32 0 1 1 64 0 32 32 0 1 1 -64 0zm192-32a32 32 0 1 1 0 64 32 32 0 1 1 0-64z" /></svg></button>
                    </div>
                    <div className='PostBtns m-1'>
                      <button className='btn btn-link'><svg xmlns="http://www.w3.org/2000/svg" height="25" width="25" viewBox="0 0 384 512"><path fill="#f80d24" d="M215.7 499.2C267 435 384 279.4 384 192C384 86 298 0 192 0S0 86 0 192c0 87.4 117 243 168.3 307.2c12.3 15.3 35.1 15.3 47.4 0zM192 128a64 64 0 1 1 0 128 64 64 0 1 1 0-128z" /></svg></button>
                    </div>
                    <div className='PostBtns m-1'>
                      <button className='btn btn-link'> <svg xmlns="http://www.w3.org/2000/svg" height="25" width="25" viewBox="0 0 576 512"><path fill="#46BD63" d="M160 32c-35.3 0-64 28.7-64 64V320c0 35.3 28.7 64 64 64H512c35.3 0 64-28.7 64-64V96c0-35.3-28.7-64-64-64H160zM396 138.7l96 144c4.9 7.4 5.4 16.8 1.2 24.6S480.9 320 472 320H328 280 200c-9.2 0-17.6-5.3-21.6-13.6s-2.9-18.2 2.9-25.4l64-80c4.6-5.7 11.4-9 18.7-9s14.2 3.3 18.7 9l17.3 21.6 56-84C360.5 132 368 128 376 128s15.5 4 20 10.7zM192 128a32 32 0 1 1 64 0 32 32 0 1 1 -64 0zM48 120c0-13.3-10.7-24-24-24S0 106.7 0 120V344c0 75.1 60.9 136 136 136H456c13.3 0 24-10.7 24-24s-10.7-24-24-24H136c-48.6 0-88-39.4-88-88V120z" /></svg></button>
                    </div>
                  </div>
                  <LoadingSpinner disable={isloading} />
                </Modal.Body>
                <Modal.Footer>
                  <button id='postbtn' className='btn btn-primary w-100 fw-bold' onClick={submitPostImages}>Post</button>
                </Modal.Footer>
              </Modal>
            }
                ...
                <Modal.Title id="contained-modal-title-vcenter" className='d-flex '>
                  <div className='PostBtns m-1'>
                    <button onClick={BackToPostModal} className='btn btn-link mt-1'>
                      <svg xmlns="http://www.w3.org/2000/svg" height="20" width="20" viewBox="0 0 448 512"><path d="M9.4 233.4c-12.5 12.5-12.5 32.8 0 45.3l160 160c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L109.2 288 416 288c17.7 0 32-14.3 32-32s-14.3-32-32-32l-306.7 0L214.6 118.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-160 160z" /></svg>
                    </button>
                  </div>
